...
if ($action === 'listar') {
  if ($tipo === 'morador') {
    $stmt = $pdo->prepare('
      SELECT u.id, u.nome, u.email, u.telefone, u.status, m.cpf, m.endereco, m.nascimento
      FROM usuarios u
    ...
    $stmt = $pdo->prepare('
      SELECT u.id, u.nome, u.email, u.telefone, u.status, p.servico, p.descricao, p.nascimento, p.status_aprovacao
      FROM usuarios u
  ...
  $stmt->execute();
  json_response(['ok' => true, 'usuarios' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
}
